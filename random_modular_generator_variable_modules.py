...
def scalefree_sequence(N, mean, seqtype):
    ...
    while tol > 0.2:
        ...
        if seqtype == "simple_degree":
            seq = [int(num * (N - 2)) + 1 for num in seq1]
        else:
...
def geometric_sequence(N, mean, seqtype):
    ...
    while (abs(mean - calc_mean)) > 0.05:
        if (mean - calc_mean) > 0.05:
            ...
            avg -= 0.1
        x = 1.0 / avg;
        seq = [(math.log(np.random.random()) / math.log(1 - x)) for i in range(N)]
...
def generate_modular_networks(N, sfunction, modfunction, Q, m, avg_degree, verbose=False, **kwds):
    ...
    while not is_valid_module_seq:
        ...
        inverted_mod_nodes = {vals: key for key in mod_nodes.keys() for vals in mod_nodes[key]}
        print(mod_nodes)
        ...
        G.add_nodes_from([(node, {'vertex_type': inverted_mod_nodes[node]}) for node in range(0, N)])
    ...
    print('modularity is equal: ' + str(Q1))
    return G, mod_nodes
...
"""
    ...
    for mod in range(len(mod_nodes.keys())):
        ...
        while tol > tolerance or (not is_valid_seq) or (tries > max_tries):
            ...
            tries += 1
            if tries > max_tries:
                raise Exception("Exceeded max (%d) attempts at a valid sequence." % max_tries)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar='number',
        type=float,
        help='how many graphs needed'
    )

    args = parser.parse_args()

    N = args.n if args.n else 512
    d = args.d if args.d else 2
    m = args.m if args.m else 2
    Q = args.q if args.q else 0.5
    ng = args.ng if args.ng else 1
    ...
    for k in range(0, ng):
        degfunction = poisson_sequence
        modfunction = regular_sequence
        try:
            ...
            G, modules = generate_modular_networks(N, degfunction, modfunction, Q, m, d)

            print("Saving adjacency matrix under the filename pd_adj_mat.csv")
            adjacency_matrix = nx.to_numpy_matrix(G)
            edges = np.where(adjacency_matrix == 1)
            df = pd.DataFrame({'vertex_1': edges[0], 'vertex_2': edges[1]})

            modules = pd.DataFrame(modules)
            modules = pd.melt(modules, value_vars=[0, 1])

            df = df.merge(modules, left_on='vertex_2', right_on='value')
